    ...
    <el-dialog
      ...
      <el-card>
        <div
          ...
          <!-- <div class="ooppOne">组织机构编码：{{ dormCode.name }}</div>
          <div class="oopp">组织机构名称：{{ dormCode.contactName }}</div>
          <div class="oopp">负责人：{{ dormCode.address }}</div>
...
export default {
  ...
  methods: {
    ...
    handlePre (row) {
      // 请求操作人列表
      this.operatorForm.organizationCode = row.code
    ...
    clickEQ (row) {
      console.log(row)
      this.organizationCode = encrypt(JSON.stringify(row))
