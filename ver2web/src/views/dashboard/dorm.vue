    ...
    <el-card>
      <el-form ref="formm"
        ...
        <el-form-item>
          ...
          <el-button type="primary"
                     size="small"
                     @click="handleAdd">新增宿舍</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    <el-card>
    ...
    <el-dialog :close-on-click-modal="false"
      ...
      <el-card>
        <div id="dormCodeID"
          ...
          <div class="ooppOne">宿舍名：{{ dormCode.name }}</div>
...
import {
  getDormitory, addDormitory, modifyDormitory,
...
export default {
  ...
  data () {
    return {
      ...
      dormFormRules: {
        ...
        longitude: [
          { required: true, message: '未定位', trigger: 'blur' }
        ]
      },
      total: 0
    }
  ...
  methods: {
    ...
    query () {
      ...
      getDormitory(this.form).then(res => {
    ...
    handleSave () {
      this.$refs.dormForm.validate(valid => {
        if (valid) {
          this.loading = true
          if (this.dorm.isEdit) {
            modifyDormitory(this.dorm).then(res => {
              this.loading = false
            ...
            addDormitory(this.dorm).then(res => {
              this.loading = false
    ...
    handler ({ BMap, map }) {
      ...
      if (this.dorm.longitude !== null && this.dorm.latitude !== null) {
        const point = new BMap.Point(this.dorm.longitude, this.dorm.address)
        console.log(point)
    ...
    handleDelete (row) {
      ...
      this.$confirm('此操作将永久删除该宿舍, 是否继续?', '提示', {
        ...
        deleteDormitory(row.id).then(res => {
          if (res.code === '0') {
            this.query()
            Message.success('删除宿舍成功')
          } else {
            Message.success('操作失败')
          }
        }).catch(err => {
          Message.error('删除宿舍失败:' + err.message)
          console.log(err)
        })
      })
    },
